...
// 由于教师精心的安排，恰好可以保证每两个小朋友都能手拉手离开队列，并且最后离开的两个小朋友是编号最小的和最大的两个小朋友。(也就是说队伍开头结尾一定是一男一女)
...
// 您应该比较字符 children[i+1] 与空格字符 ' '，而不是字符串 " "。
// 字符用单引号，字符串用双引号

int leave(char children[], int girl);
...
int leave(char str[], int idx){
    if(str[idx] != str[0]){
        return idx;
    }else{
        int boyidx = idx;
        int girlidx = leave(str,boyidx+1);
        cout<<boyidx<<' '<<girlidx<<endl;
        return leave(str, girlidx+1);
    }
}
